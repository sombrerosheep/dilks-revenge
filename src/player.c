...
static int Player_Shoot(Vec2 pos, Vec2 vel) {
    Projectile               pro;
    ProjectileGameContainer *pgcon = ResourceManager_GetProjectileContainer(&GameResources);

    Projectile_Init(&pro, ProjectileType_Player, pos, vel, 450.f);
    GameContainer_Insert(Projectile, pgcon, pro);
